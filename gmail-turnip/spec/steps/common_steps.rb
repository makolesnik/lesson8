module Turnip::Steps
  # GIVEN
  step 'I am on the accounts.google.com sign up page' do
    @sp = SignUpPage.new
    @sp.load
  end
  # WHEN

  step 'I fill the sign up form with valid credentials and submit' do
  ...
  step 'I fill the sign up form with already used phone and submit' do
  ...
  step 'I fill the sign up form with incorrect credentials and submit' do
  ...
  step 'I should see welcome page notification :text' do |text|
    @wp = WelcomePage.new
    expect(@wp.welcome.text.include? text).to be_truthy
  end

  step 'I should see current page notification :text' do |text|
    expect(@vp.phone_used_error.text.include? text).to be_truthy
  end

  step 'the sign up should be successfully' do
    expect(@wp.new_email.text.include? @user.username).to be_truthy
  end

  step 'the sign up should be failed' do
  ...
  step 'I can sign in to my new gmail account' do
    expect(GmailHelper.verify_connected(@user)).to be_truthy
  end

  step 'I can`t sign in to my new gmail account' do
